...
function LeadFollowupSelectionDiv({ methodways, student, handleMethodSelect, selectedMethod,StageValue,higestvalueContanorIndex }) {
    const studentLeadLength=student?.student_leads?.length  
    
    return (
        <div className="space-y-4">
            <h1 className="text-2xl font-semibold text-center mb-4 border-b">Send Follow-up Message stage {studentLeadLength===0 ? 0:
            StageValue}</h1>
            <div className="flex flex-row  justify-around gap-4 ">
                {methodways.map((method) => (
                    <div
                        ...
                        ${studentLeadLength>0 ?(`${(student?.student_leads[higestvalueContanorIndex][method?.isname]==='Yes') ? 'pointer-events-none' : ''}`):''}`}
                        onClick={() => handleMethodSelect(method.name)}
                    >
                        <span className="text-lg font-semibold">{method.label}</span>
                        {(studentLeadLength>0 ?(student?.student_leads[higestvalueContanorIndex][method?.isname]==='Yes') :'')&&(
